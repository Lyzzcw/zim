...
(
    `id`      bigint(20) NOT NULL AUTO_INCREMENT COMMENT 'id',
    `message_code` bigint(20) NOT NULL COMMENT '消息码',
    `cmd_type` tinyint(1) NOT NULL COMMENT '消息类型',
    `send_id` bigint(20) NOT NULL COMMENT '发送用户id',
    `recv_id` bigint(20) NOT NULL COMMENT '接收用户id(群消息则为群id)',
    `content` text CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NULL COMMENT '发送内容',
    `message_type`    tinyint(1) NOT NULL COMMENT '消息内容类型 0:文字 1:图片 2:文件 3:语音',
  ...
  INDEX `idx_message_code`(`message_code`) USING BTREE
) CHARACTER SET = utf8mb4 COLLATE = utf8mb4_general_ci COMMENT = '消息列表';
-- ----------------------------
